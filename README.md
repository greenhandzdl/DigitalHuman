...
./run.sh kbslice            # 把外部那包 .docx 语料切成知识库语料（语料本身不入库；换语料才跑）
...
快速开始里那三条 `kb*` 命令是三段不同的事，各有一组判据，别把它们当同一条：

| 层次 | 命令 | 判的是 |
|---|---|---|
| 语料切得开 | `./run.sh kbslice` | 一份 `.docx` 语料包切成**带章节路径**的片段 —— 上游那套定长切片会把"这张表在讲什么"那级标题和表体劈开，所以先按原序遍历段落与表格再攒块 |
| 检索得回来 | `./run.sh kb` | 切片入库 + 拿真实问法抽测，要求每问在 top3 命中自己的出处 |
| 业务里真用上了 | `./run.sh kbq` | 从 Fay 的业务口问一句，判那一问**落库的原始回帧**里知识库有没有被注入 |

第三条必须单独存在：前两条都是**直调 MCP 工具**，它们绿只说明"库和路没问题"，说明不了真实
那一轮里用户在听谁说话。而 `kbq` 读的是 `:5000` 那一行的原文 —— `<prestart>` 那对标签在 Fay
的三个出口里有两个会明写剥掉，只有落库那行留着。

一条机制，部署前就该知道：**查不查知识库不是模型决定的**。`query_yueshen` 注册成 prestart
工具（`{"query":"{{question}}","top_k":3}`），Fay 在拼 prompt 之前无条件跑一次，结果作为上下文
注入 —— 所以注入是确定发生的。但**模型引不引用原文不是**：同一问、同一份注入连跑三次，正文
逐字复现知识库那句话只有一次，另两次答的是它自己的通用常识。所以那组判据里"注入块含期望词"
是硬判、"正文复现"只报不判红（否则判的是采样脾气）。顺带一条运维事实：想临时停掉注入，
在管理台把工具「禁用」是**没用的**（预启动调用带 `skip_enabled_check=True`），能掐断的只有摘注册。

`kbq` 一组要发两整轮问答、每轮都过一遍大模型，所以它不进 `./run.sh test` 的常态清单，
由人点名发起（`./run.sh test kb-fay` 是同一条判据）。语料本身是项目方给的，不进本仓库。
